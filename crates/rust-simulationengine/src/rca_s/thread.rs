    ...
    pub fn step(&mut self, ctx: &mut Data) -> Result<(), Error> {
        match self {
            ProgramThread::Main { counter, tasks, handoff } => {
                ...
                // Transfer ownership of current handoff into the cell
                let handoff_transfer: CellData = std::mem::take(handoff);

                // Execute current cell
                let out: (CellData, Result<TaskOutput, Error>) =
                    tasks[*counter].execute(ctx, handoff_transfer);

                let next_handoff = out.0;
                let task_output = out.1?;

                // Thread owns continuation policy
                *handoff = match task_output {
                    TaskOutput::None => CellData::None,
                    TaskOutput::NextCell => next_handoff,
                    TaskOutput::CommitTick => next_handoff,
                    _ => CellData::None,
                };
